<?php
$target_dir = "../uploads/";
$target_file = $target_dir . basename($_FILES["fileToUpload_sk"]["name"]);
$uploadOk = 1;
$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

// is image real
if(isset($_POST["add"])) {
    $check = getimagesize($_FILES["fileToUpload_sk"]["tmp_name"]);
    if($check !== false) {
...
// Check file size
if ($_FILES["fileToUpload_sk"]["size"] > 99500000) {
...
// если все в порядке, поробуем загрузить файл
    ...
    if (move_uploaded_file($_FILES["fileToUpload_sk"]["tmp_name"], $target_file)) {
        echo "Файл ". htmlspecialchars( basename( $_FILES["fileToUpload_sk"]["name"])).
            " був завантажений";
    } else {
